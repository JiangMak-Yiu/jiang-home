...
测试站点个人导航站源码。PHP 8.2 + SQLite,零配置,开箱即用。

原站为纯静态 HTML,现已全部数据动态化:前台展示、后台管理、友链申请审核,不装任何依赖即可跑起来。
...
- 后台管理(`/admin/`):
  ...
  - 友链管理(增删改,支持封面图)
  - 友链申请(前台提交,后台审核)
  - 站点设置(Hero 文案、公告、QQ/TG、ICP 备案等)
  - 数据备份(一键导出 SQLite 数据)
  - 修改密码
- 安全:CSRF 防护、登录失败 5 次锁 15 分钟、会话 30 分钟超时、友链申请蜜罐 + 60 秒限频
...
```bash
# 1. 克隆代码到站点目录
git clone https://github.com/JiangMak-Yiu/jiang-home.git /www/wwwroot/jiang-home
